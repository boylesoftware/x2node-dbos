...
class PostgreSQLDBDriver extends BasicDBDriver {
	...
	updateVersionTable(
		...
		connection.query(sql, (err, result) => {
			...
			sql = `INSERT INTO ${tableName} (name, modified_on, version)`;
			for (let i = 0, len = itemNames.length; i < len; i++) {
				if (i++ > 0)
					sql += ',';
				sql += ' VALUES (' + this.stringLiteral(itemNames[i]) +
					', \'' + modificationTimestamp + '\', 1)';
			}
			sql += ' ON CONFLICT name DO UPDATE SET modified_on = \'' +
			...
			/*trace(sql = `LOCK TABLE ${tableName} IN EXCLUSIVE MODE`);
			connection.query(sql, err => {

				if (err)
					return handler.onError(err);

				trace(sql = `SELECT name FROM ${tableName} WHERE ${filterExpr}`);
				connection.query({
					text: sql,
					rowMode: 'array'
				}, (err, result) => {
					...
					const newItemNames = new Set(itemNames);
					for (let r of result.rows)
						newItemNames.delete(r[0]);

					sql =
						`INSERT INTO ${tableName} (name, modified_on, version)`;
					let i = 0;
					newItemNames.forEach(itemName => {
						if (i++ > 0)
							sql += ',';
						sql += ' VALUES (' +
							this.stringLiteral(itemName) +
							', \'' + modificationTimestamp + '\', 1)';
					});
					trace(sql);
					connection.query(sql, err => {
