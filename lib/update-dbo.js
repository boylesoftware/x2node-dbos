...
class UpdateDBO extends AbstractDBO {
	...
	_parsePatchOperation(recordsPropDesc, patchOp, opInd) {

		// validate the op
		if ((typeof patchOp.op) !== 'string')
			throw new common.X2UsageError(
				'Invalid patch operation #' + (opInd + 1) +
		...
		case 'add':
			pathInfo = this._resolvePropPointer(
				recordsPropDesc, patchOp.path, true);
		...
		case 'remove':
			pathInfo = this._resolvePropPointer(
				recordsPropDesc, patchOp.path, false);
			//...
			break;
		case 'replace':
			pathInfo = this._resolvePropPointer(
		...
		case 'move':
			...
			fromInfo = this._resolvePropPointer(
				recordsPropDesc, patchOp.from, false);
			this._validatePatchOperationFrom(
				pathOp.op, opInd, pathInfo, fromInfo);
			//...
			break;
		case 'copy':
			pathInfo = this._resolvePropPointer(
	...
	_resolvePropPointer(recordsPropDesc, propPointer, allowDash) {
		...
		for (let i = 1, len = propPointerTokens.length; i < len; i++) {
			...
			if (!inCol && lastPropDesc.isArray()) {
				...
				if ((i > 1) && (lastPropDesc.scalarValueType !== 'object'))
					throw new common.X2UsageError(
						'Invalid property pointer "' + propPointer +
							'" in patch operation: ' +
							lastPropDesc.container.nestedPath +
							lastPropDesc.name +
				...
				lastPropDesc = container.getPropertyDesc(propPointerToken);
				inCol = false;
			}
		}

		// return the resolved pointer info
		return {
			propDesc: lastPropDesc,
			propPath: lastPropDesc.container.nestedPath + lastPropDesc.name,
			collectionElement: inCol
		};
	}
	...
	_validatePatchOperationValue(opType, opInd, pathInfo, val, forUpdate) {
		...
		const validate = errMsg => {
			if (errMsg)
				new common.X2UsageError(
					'Invalid value in ' + opType + ' patch operation #' +
						(opInd + 1) + ': ' + errMsg);
		};
		...
		if (propDesc.isArray() && !pathInfo.collectionElement) {
			...
			val.forEach(v => {
				validate(this._isValidScalarValueType(v, propDesc, forUpdate));
			});
			...
			keys.forEach(k => {
				validate(this._isValidScalarValueType(
					val[k], propDesc, forUpdate));
			});
		} else {
			validate(this._isValidScalarValueType(val, propDesc, forUpdate));
		}
	}

	/**
	 * Tell if the specified value is good as a value for the specified property
	 * as a scalar (so if the property is not scalar, tests if the value is good
	 ...
	 */
	_isValidScalarValueType(val, propDesc, forUpdate) {
	...
	_isValidObjectValue(val, objectPropDesc, forUpdate) {
		...
		for (let propName of objectPropDesc.allPropertyNames) {
			...
			if ((propVal === undefined) || (propVal === null)) {
				...
				if (!propDesc.optional && (propVal.length === 0))
					return false;
				if (!propVal.every(
					v => this._isValidScalarValueType(v, propDesc, forUpdate)))
					return false;
				...
				if (!propDesc.optional && (keys.length === 0))
					return false;
				if (!keys.every(
					k => this._isValidScalarValueType(
						propVal[k], propDesc, forUpdate)))
					return false;
			} else {
				if (!this._isValidScalarValueType(propVal, propDesc, forUpdate))
					return false;
			}
		}
	...
	_validatePatchOperationFrom(opType, opInd, pathInfo, fromInfo) {
		...
		const invalidFrom = msg => new common.X2UsageError(
			'Invalid from pointer in ' + opType + ' patch operation #' +
				(opInd + 1) + ': ' + msg);
