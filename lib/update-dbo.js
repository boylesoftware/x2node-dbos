...
class UpdateColumnCommand extends Command {
	...
	prepare(ctx, record) {
		...
		const curVal = this._pathPtr.getValue(record, true);
...
class PropertyPointer {
	...
	getValue(record, forUpdate) {
		...
		return pointerChain.reduceRight(
			(obj, p, i) => p._getImmediateValue(obj, i, forUpdate), record);
	}
...
class UpdateDBO extends AbstractDBO {
	...
	_parsePatchOperation(recordsPropDesc, patchOp, opInd, involvedPropPaths) {
		...
		switch (patchOp.op) {
		case 'add':
			pathPtr = this._resolvePropPointer(
		...
		case 'move':
			pathPtr = this._resolvePropPointer(
				recordsPropDesc, patchOp.path, true, true);
			fromPtr = this._resolvePropPointer(
	 ...
	 *
	 * @private
	 * @param {module:x2node-dbos~PropertyPointer} pathPtr Resolved property
	 * pointer.
	 * @param {Set.<string>} involvedPropPaths Involved property paths
	 * collection.
	 */
	 ...
	 * operations.
	 * @param {module:x2node-dbos~PropertyPointer} pathPtr Information object
	 ...
	 * @param {module:x2node-dbos~PropertyPointer} fromPtr Information object
