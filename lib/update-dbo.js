...
class UpdateDBOExecutionContext extends DBOExecutionContext {
	...
	onSet(op, ptr, newValue, oldValue) {
		...
		// build the anchors conditions and find the last container
		// TODO: stop on the first table-wide unique id anchor
		const anchors = new Array();
		let containerDesc, containerObj, fullArray;
		ptr.getValue(this._record, (prefixPtr, value, prefixDepth) => {
			if (prefixPtr.isRoot()) {
				...
				if (!containerDesc) {
					containerDesc = this._recordTypeDesc;
					containerObj = this._record;
				}
				...
				if (prefixPtr.collectionElement) {
					if (propDesc.isArray()) {
						...
						anchors.push({
							...
							// TODO: transform value type if ref, for example
							valueExpr: this._dbDriver.sql(
								prefixPtr.collectionElementIndex)
						});
					}

				} else if (propDesc.isArray() && !fullArray) {
					fullArray = value;
				}

				if (!containerDesc && (prefixDepth > 0) &&
					(propDesc.scalarValueType === 'object') &&
					propDesc.nestedProperties.idPropertyName) {
					containerDesc = propDesc.nestedProperties;
					containerObj = value;
				}
			}
		});
		...
		if (propDesc.scalarValueType === 'object') {
			...
			if (newValue && (newValue.length > 0))
				this._commands.push(new PopulateSimpleArrayCommand(
					propDesc.table, anchors, propDesc.parentIdColumn,
					this._dbDriver.sql(
						containerObj[containerDesc.idPropertyName]),
			...
			const propValColumnInfo = this._translationCtx.getPropValueColumn(
				ptr.propPath + '.$value');
			const propKeyColumnInfo = this._translationCtx.getPropValueColumn(
				ptr.propPath + '.$key');
			...
			if (newKeys && (newKeys.length > 0)) {
				this._commands.push(new PopulateSimpleMapCommand(
					...
					this._dbDriver.sql(
						containerObj[containerDesc.idPropertyName]),
					propKeyColumnInfo.columnName, propValColumnInfo.columnName,
					newKeys.map(k => [
						this._dbDriver.sql(k), this._dbDriver.sql(newValue[k])
					])
				));
			}
			...
			this._commands.push(new PopulateSimpleArrayCommand(
				propDesc.table, anchors, propDesc.parentIdColumn,
				this._dbDriver.sql(containerObj[containerDesc.idPropertyName]),
				propValColumnInfo.columnName,
				fullArray.map(v => this._dbDriver.sql(v))
			));
