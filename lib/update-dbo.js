		...
		return promiseChain.then(
			() => {
				...
				for (let rec = ctx.nextRecord(); rec; rec = ctx.nextRecord()) {
					...
					if (ctx.recordValidator) {
						recordsChain = recordsChain.then(
							() => ctx.recordValidator(rec),
							err => Promise.reject(err)
						);
					}

					// flush updates to the dataase
...
class UpdateDBOExecutionContext extends DBOExecutionContext {
	...
	constructor(dbo, txOrCon, actor, filterParams, recordValidator) {
		...
		this._recordValidator = recordValidator;
	 ...
	 * @readonly
	 */
	get recordValidator() { return this._recordValidator; }
...
class UpdateDBO extends AbstractDBO {
	 ...
	 */
	constructor(
		dbDriver, recordTypes, recordCollectionsTracker, recordTypeDesc, patch,
		filterSpec) {
		super(dbDriver, recordTypes, recordCollectionsTracker);
		...
		// add record collection updates tracker command
		this._commands.push(this._createCallRecordCollectionTrackersCommand());
	...
	execute(txOrCon, actor, recordValidator, filterParams) {
		...
		return this._executeCommands(new UpdateDBOExecutionContext(
			this, txOrCon, actor, filterParams, recordValidator));
	}
}
