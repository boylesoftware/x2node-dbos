...
class QueryFilterCollectionTest extends QueryFilter {
	 ...
	 * tree.
	 * @param {string} colPropPath Collection property path from the filter
	 * specification.
	...
	constructor(
		...
		this._colBasePath = valueExprCtx.basePath.substring(
			0, valueExprCtx.basePath.length -
				(colPropPath.length - colBasePath.length));
...
function buildFilter(
	...
	if (singlePropDesc && !singlePropDesc.isScalar()) {
		...
		const basePathLen = valueExprCtx.basePath.length;
		if (predParts[2] !== (
			basePathLen > 0 ?
				singlePropPath.substring(basePathLen + 1) : singlePropPath))
			throw error(
