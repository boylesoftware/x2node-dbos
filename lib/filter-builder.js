...
class QueryFilter {
	 ...
	 * Create new filter.
	 *
	 * @param {boolean} empty <code>true</code> if initially empty.
	 * @param {boolean} needsParen <code>true</code> if initially needs
	 * parenthesis.
	 */
	constructor(empty, needsParen) {

		this._empty = empty;
		this._needsParen = needsParen;
	}
	 ...
	 *
	 * @param {module:x2node-queries.DBDriver} dbDriver The DB driver.
	 * @param {Function} propsResolver The properties resolver function.
	 * @param {Object.<string,Function>} funcResolvers Expression function
	 * resolvers.
...
class QueryFilterJunction extends QueryFilter {
	...
	translate(dbDriver, propsResolver, funcResolvers, paramsHandler) {
		...
		const juncSql = this._elements.map(element => {
			...
			const needsParen = (
				(element instanceof QueryFilterJunction) &&
					(this._elements.length > 1) &&
					(element._juncType !== this._juncType) &&
					element.needsParen()
			);
			return (needsParen ? '(' + elementSql + ')' : elementSql);
		}).join(' ' + this._juncType + ' ');
...
class QueryFilterCollectionTest extends QueryFilter {
	 ...
	 */
	constructor(
		recordTypes, propsTreeBuilderFunc, colPropPath, invert, topPropDesc,
		...
		this._propsTree = propsTreeBuilderFunc(colPropPath, usedPropPaths);

		// find all properties used outside the collection context
		const dotInd = colPropPath.indexOf('.');
		const colBasePath = (
			dotInd < 0 ? colPropPath : colPropPath.substring(0, dotInd)) + '.';
		usedPropPaths.forEach(propPath => {
			if (!propPath.startsWith(colBasePath))
	...
	translate(dbDriver, propsResolver, funcResolvers, paramsHandler) {
		...
		const subqueryTree = queryTreeBuilder.buildIdsOnly(
			dbDriver, this._recordTypes, this._propsTree);
		console.log('>>> subquery: ' + subqueryTree.assembleSelect(this._colFilter, null, paramsHandler));
