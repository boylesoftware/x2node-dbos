...
class QueryTreeNode {
	 ...
	 */
	constructor(
		dbDriver, recordTypes, propsSql, delayedJoinConditions, singleAxis,
		propNode, collection, table, tableAlias, keyColumn) {
	...
	cloneWithoutChildren() {
		...
		return new QueryTreeNode(
			...
			new Map(), this._singleAxis, this._propNode, this._collection,
			this._table, this._tableAlias, this._keyColumn);
	}
	...
	createChildNode(
		...
		const childNode = new QueryTreeNode(
			...
			this._delayedJoinConditions, this._singleAxis, propNode, many, table,
			this._childTableAliasPrefix +
...
function buildQueryTree(
	...
	const topNode = (
		anchorNode ?
			...
			new QueryTreeNode(
				dbDriver, recordTypes, new Map(), new Map(), singleAxis,
				propsTree, true, recordTypeDesc.table, 'z', topIdColumn)
	);
...
exports.forAnchoredQuery = function(
	...
	const anchorNode = new QueryTreeNode(
		dbDriver, recordTypes, new Map(), new Map(), singleAxis, propsTree, true,
		anchorTable, 'q',
...
exports.forSuperPropsQuery = function(dbDriver, recordTypes, superPropsTree) {
	...
	const superNode = new QueryTreeNode(
		dbDriver, recordTypes, new Map(), new Map(), true, superPropsTree, false,
