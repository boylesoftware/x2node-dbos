...
class QueryTreeNode {
	 ...
	 * ORDER BY clause.
	 * @param {?string} [lockType] Lock type: "shared" or "exclusive". If
	 * "exclusive", downgraded to "shared" when references are crossed over.
	 * @param {module:x2node-dbos~FilterParamsHandler} paramsHandler DBO
	 * parameters handler.
	 * @returns {string} The query SQL.
	 */
	assembleSelect(filter, order, lockType, paramsHandler) {
		...
		const queryBuilder = {
			...
			select: new Array(),
			from: null,
			properTables: new Set(),
			referredTables: new Set(),
			where: null,
			groupBy: new Array(),
			orderBy: new Array()
		};
		...
		// translation context
		const ctx = this.getTopTranslationContext(paramsHandler);
		...
		this._addNodeToSelect(queryBuilder, false, false, false, ctx);
		...
		// determine tables to place locks
		let exclusiveLockTables, sharedLockTables;
		switch (lockType) {
		case 'exclusive':
			exclusiveLockTables = Array.from(queryBuilder.properTables);
			sharedLockTables = Array.from(queryBuilder.referredTables);
			break;
		case 'shared':
			queryBuilder.properTables.forEach(
				t => { queryBuilder.referredTables.add(t); });
			sharedLockTables = Array.from(queryBuilder.referredTables);
		}

		// check if database does not support locks with aggregates
		if (!ctx.dbDriver.supportsRowLocksWithAggregates() &&
			queryBuilder.hasAggregates && (
				(exclusiveLockTables && (exclusiveLockTables.length > 0)) ||
					(sharedLockTables && (sharedLockTables.length > 0))
			))
			throw new Error(
				'Internal X2 error: explicit row locking in a query with' +
					' aggregates is not supported by the database.');

		// assemble the query and return it
		return ctx.dbDriver.makeSelectWithLocks(
			'SELECT ' +
			...
			exclusiveLockTables, sharedLockTables
		);
	}
	 ...
	 * @param {boolean} inReferred <code>true</code> if a reference property has
	 * been crossed over.
	 * @param {module:x2node-dbos~TranslationContext} ctx Translation context.
	 */
	_addNodeToSelect(queryBuilder, forceOuter, aggregated, inReferred, ctx) {

		// mark query as having aggregates if aggregated
		if (aggregated)
			queryBuilder.hasAggregates = true;
		...
		const isTopNode = !this[PARENT_NODE];
		if (isTopNode) {
			...
			queryBuilder.from +=
				...
				' ON ' + this._buildFullJoinCondition(ctx);
		}

		// add node's table to the appropriate tables group for locking
		if (inReferred) {
			queryBuilder.referredTables.add(this._table);
		} else if (isTopNode) {
			queryBuilder.properTables.add(this._table);
		} else {
			inReferred = (
				this._propNode.desc.isRef() &&
					(this._propNode.desc.table === this._table));
			if (inReferred)
				queryBuilder.referredTables.add(this._table);
			else
				queryBuilder.properTables.add(this._table);
		...
		// add children
		this._allChildren.forEach(childNode => {
			childNode._addNodeToSelect(
				queryBuilder, (forceOuter || this._virtual),
				(aggregated || this._aggregatedBelow), inReferred, ctx)
		});
	}
	...
	assembleExistsSubquery(filter, paramsHandler) {
		...
		// process query tree nodes
		subqueryNode._addNodeToExistsSubquery(queryBuilder, ctx);
	 ...
	 * @param {module:x2node-dbos~TranslationContext} ctx Translation context.
	 */
	_addNodeToExistsSubquery(queryBuilder, ctx) {
		...
		// add children
		this._allChildren.forEach(childNode => {
			childNode._addNodeToExistsSubquery(queryBuilder, ctx)
		});
	}
