...
class QueryTreeNode {
	...
	addProperty(propNode, clauses, markupCtx) {
		...
		case 'map:datetime':
			...
			if (propDesc.isAggregate()) {
				...
				// save key column and SQL mapping
		...
		if (propDesc.isPolymorphObject() &&
			propDesc.nestedProperties.hasProperty('$type')) {
			const typePropPath = propNode.path + '.$type';
			const typeColumn = propDesc.nestedProperties.getPropertyDesc(
				'$type').column;
			this.addPropValueColumn(
				typePropPath, queryTreeNode.table,
				queryTreeNode.tableAlias, typeColumn);
			this.addPropSql(
				typePropPath, queryTreeNode.tableAlias + '.' + typeColumn);
		}
	}
	...
	_addRefPropertyViaLinkTable(
		...
		// save key column and SQL mappings
