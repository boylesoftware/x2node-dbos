...
class TranslationContext {
	 ...
	 * calculated or dependent reference properties are. For collection
	 * properties the value column can be found by adding ".$value" to the
	 * property path. For maps, the key column can be found by adding ".$key" to
	 * the property path.
	 *
	 * @param {string} propPath Property path.
	 * @returns {Object} Column information object that includes properties
	 * <code>tableAlias</code> and <code>columnName</code>.
	 */
...
class QueryTreeNode {
		...
		case 'map:datetime':
			...
			if (propDesc.isAggregate()) {
				...
				this.addPropValueColumn(
					valPath, queryTreeNode.tableAlias, propDesc.column);
				...
				if (propDesc.isMap()) {
					...
					this.addPropValueColumn(
						keyPath, queryTreeNode.tableAlias, propDesc.keyColumn);
				}
	...
	_addScalarSimpleProperty(
		...
		// check if calculated
		let valueSelector, valueTableAlias;
		const propDesc = propNode.desc;
		if (propDesc.isAggregate()) {
			...
			// value column info
			valueTableAlias = queryTreeNode.tableAlias;

			// create value selector
			valueSelector = makeSelector(
				valueTableAlias + '.' + valueColumn,
		...
		if (valueTableAlias)
			this.addPropValueColumn(propNode.path, valueTableAlias, valueColumn);
	...
	_addRefPropertyViaLinkTable(
		...
		let queryTreeNode, valTableAlias, valColumn, keyTableAlias, keyColumn;
		const propDesc = propNode.desc;
		...
		// make and save value and key mappings
		...
		this.addPropValueColumn(valPath, valTableAlias, valColumn);
		if (propDesc.isMap()) {
			const keyPath = propNode.path + '.$key';
			this.addPropSql(keyPath, keySql);
			this.addPropValueColumn(keyPath, keyTableAlias, keyColumn);
		}
	...
	assembleSelect(filter, order, paramsHandler) {
		...
		this._addNodeToSelect(queryBuilder, false, false, ctx);
	 ...
	 * @param {module:x2node-dbos~TranslationContext} ctx Translation context.
	 */
	_addNodeToSelect(queryBuilder, forceOuter, aggregated, ctx) {
	 ...
	 */
	addPropValueColumn(propPath, tableAlias, columnName) {
...
function buildQueryTree(
	...
	topNode.addPropValueColumn(topIdPropName, topNode.tableAlias, topIdColumn);
