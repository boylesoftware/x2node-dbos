...
class QueryTreeNode {
	 ...
	 */
	constructor(
		dbDriver, recordTypes, propsSql, propPath, collection, table, tableAlias,
		keyColumn) {
	...
	cloneWithoutChildren() {
		...
		return new QueryTreeNode(
			...
			this._propPath, this._collection, this._table, this._tableAlias,
			this._keyColumn);
	}
	...
	createChildNode(
		...
		const childNode = new QueryTreeNode(
			this._dbDriver, this._recordTypes, this._propsSql, propPath, many,
			table,
			this._childTableAliasPrefix +
		...
		if (propNode.valueExpr) {
			...
			if (propNode.aggregateFunc) {
				valueSelector = makeSelector(
					ctx => propNode.aggregateFunc + '(' +
						propNode.valueExpr.translate(ctx) + ')',
	...
	addNodeToSelect(queryBuilder, forceOuter, aggregated, ctx) {
		...
		this._allChildren.forEach(childNode => {
			childNode.addNodeToSelect(
				...
				(aggregated || this._aggregated), ctx)
		});
	}
...
function buildQueryTree(dbDriver, recordTypes, propsTree, anchorNode, clauses) {
	...
	const topNode = (
		anchorNode ?
			...
			new QueryTreeNode(
				dbDriver, recordTypes, new Map(), propsTree.path, true,
...
exports.buildAnchored = function(
	...
	const anchorNode = new QueryTreeNode(
		dbDriver, recordTypes, new Map(), propsTree.path, true, anchorTable, 'q',
