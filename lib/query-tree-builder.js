...
class QueryTreeNode {
	...
	addProperty(propNode, clauses, markupCtx) {
		...
		case 'array:datetime:mono':
			...
			this._addCollectionSimpleProperty(
				propNode, select, markupPrefix, childrenMarkupCtx.prefix,
				propDef.table, propDef.parentIdColumn, this.keyColumn,
	...
	assembleSelect(filter, order, paramsHandler) {
		...
		const propsResolver = propPath => this._propsSql.get(propPath);

		// add top filter if any
		if (filter)
			queryBuilder.where = filter.translate(
				this._dbDriver, propsResolver, VALUE_EXPR_FUNCS, paramsHandler);
		...
		this._addNodeToSelect(
			queryBuilder, null, this, false, propsResolver, paramsHandler);
	...
	_addNodeToSelect(
		queryBuilder, parentQueryTreeNode, queryTreeNode, forceOuter,
		propsResolver, paramsHandler) {

		// add SELECT clause elements
		queryTreeNode._select.forEach(s => {
		...
		if (parentQueryTreeNode) {
			let joinCondition =
				queryTreeNode._tableAlias + '.' + queryTreeNode._joinByColumn +
				' = ' + parentQueryTreeNode._tableAlias + '.' +
				queryTreeNode._joinToColumn;
			if (queryTreeNode._joinCondition) {
				// TODO: add scoped join condition
			}
			queryBuilder.from +=
				(
					forceOuter || queryTreeNode._virtual ?
						' LEFT OUTER' : ' INNER'
				) + ' JOIN ' + queryTreeNode._table + ' AS ' +
				queryTreeNode._tableAlias + ' ON ' + joinCondition;
		} else { // top node
			queryBuilder.from =
				queryTreeNode._table + ' AS ' + queryTreeNode._tableAlias;
		}

		// add order
		queryTreeNode._order.forEach(o => {
			queryBuilder.orderBy.push((
		...
		queryTreeNode.children.forEach(childNode => {
			this._addNodeToSelect(
				queryBuilder, queryTreeNode, childNode,
				(forceOuter || queryTreeNode._virtual), propsResolver,
				paramsHandler)
		});
	}
...
exports.buildExistsSubquery = function(dbDriver, recordTypes, propsTree) {
