...
class TranslationContext {
	...
	getPropSql(propPath) { return this._propsSql.get(propPath); }
...
class QueryTreeNode {
	 ...
	 */
	constructor(
		dbDriver, recordTypes, propsSql, propPath, table, tableAlias,
		keyColumn) {
	...
	cloneWithoutChildren() {
		...
		return new QueryTreeNode(
			...
			this._propPath, this._table, this._tableAlias, this._keyColumn);
	}

	setTableAlias(tableAlias, childTableAliasPrefix) {

		this._tableAlias = tableAlias;
		this._childTableAliasPrefix = (childTableAliasPrefix || tableAlias);
	}
	 ...
	 */
	createChildNode(
		propPath, table, keyColumn, expanding, collection, virtual, joinByColumn,
		joinToColumn, joinCondition, order) {

		const childNode = new QueryTreeNode(
			this._dbDriver, this._recordTypes, this._propsSql, propPath, table,
		...
		if (order)
			order.forEach(o => {
				// TODO: optimize: don't include if same as parent id
				childNode._order.push(makeOrderElement(
	...
	assembleSelect(filter, order, paramsHandler) {
		...
		this.addNodeToSelect(queryBuilder, false, ctx);
	 ...
	 */
	addNodeToSelect(queryBuilder, forceOuter, ctx) {
		...
		this._allChildren.forEach(childNode => {
			childNode.addNodeToSelect(
				queryBuilder, (forceOuter || this._virtual), ctx)
		});
	}
...
function buildQueryTree(dbDriver, recordTypes, propsTree, anchorNode, clauses) {
	...
	const topNode = (
		anchorNode ?
			...
			new QueryTreeNode(
				dbDriver, recordTypes, new Map(), propsTree.path,
				recordTypeDef.table, 'z', topIdColumn, topIdPropName)
	);

	// fix top table alias
	if (anchorNode)
		topNode.setTableAlias('z');
...
exports.buildAnchored = function(
	...
	const anchorNode = new QueryTreeNode(
		dbDriver, recordTypes, new Map(), propsTree.path, anchorTable, 'q',
