...
class QueryFactory {
	...
	buildFetch(recordTypes, recordTypeName, querySpec) {
		...
		return new FetchQuery(
			this._dbDriver, recordTypes, superTypeDesc, selectedPropPatterns,
	...
	_getSuperRecordTypeDesc(recordTypes, recordTypeName) {
		...
		if (recordTypes.hasRecordType(superTypeName)) {
			...
			superTypeDesc.recordsRecordTypeName = recordTypeName;
		}
