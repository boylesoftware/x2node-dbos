...
class QueryFactory {
	 ...
	 */
	buildFetch(recordTypes, topRecordTypeName, querySpec) {

		// validate top record type descriptor
		...
		const querySpecObj = new QuerySpec(
			recordTypes,
			recordTypes.getRecordTypeDesc(
				this._getSuperRecordTypeName(recordTypes, topRecordTypeName)
			).getPropertyDesc('records'),
			new ValueExpressionContext(
				'', [ recordTypes.getRecordTypeDesc(topRecordTypeName) ]),
			(querySpec ? querySpec : {}));

		// build and return the query
		return new Query(this._dbDriver, topRecordTypeName, querySpecObj);
	}

	/**
	 * Get super record type name for the specified record type. If super record
	 * type has not been registered yet, create and register it.
	 *
	 * @private
	 * @param {module:x2node-records~RecordTypesLibrary} recordTypes Record types
	 * library.
	 ...
	 * @returns {external:Symbol} Super record type name symbol.
	 */
	_getSuperRecordTypeName(recordTypes, recordTypeName) {
		...
		if (!recordTypes.hasRecordType(superTypeName)) {
			...
			const recordsPropDef = Object.create(recordTypeDesc.definition);
			recordsPropDef.valueType = '[object]';
			recordsPropDef.optional = false;
			recordsPropDef.parentIdColumn = '\'' + recordTypeName + '\'';

			// TODO: add super-views
