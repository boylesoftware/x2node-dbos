 ...
 */
class QueryOrder extends Translatable {
	 ...
	 *
	 * @param {module:x2node-queries~QueryOrderElement} element Element to add.
	 * @returns {module:x2node-queries~QueryOrder} This order specification.
	 */
	addElement(element) {
	 ...
	 * Order elements.
	 *
	 * @type {module:x2node-queries~QueryOrderElement[]}
	 * @readonly
	 */
 ...
 * @extends module:x2node-queries~Translatable
 */
class QueryOrderElement extends Translatable {

	/**
	 * Create new element.
	 *
	 * @param {module:x2node-queries~ValueExpression} valueExpr Value exression.
	...
	/**
	 * Tell if ordering in descending order.
	 *
	 * @returns {boolean} <code>true</code> if in descending order.
	 */
	 ...
	 * @returns {string} SQL expression for the order element.
	 */
	translate(ctx) {
...
/**
 * Parse query order specification and build the order.
 *
 * @private
 ...
 * @returns {module:x2node-queries~QueryOrder} Parsed order specification, or
...
exports.buildOrder = function(valueExprCtx, orderSpecs) {
	...
	return orderSpecs.reduce((order, elementSpec) => {
		...
		return order.addElement(new QueryOrderElement(
