...
class PropertyTreeNode {
	...
	addChild(
		...
		if (propDef.aggregate) {
			...
			childNode._aggregatedPropPath = (
				this._path.length > 0 ?
					this._path + '.' + aggColPath : aggColPath);
			const aggColContainerChain = Array.from(this._containerChain);
			aggColPath.split('.').forEach(p => {
				const c = aggColContainerChain[aggColContainerChain.length - 1];
				if (!c.hasProperty(p))
					throw invalidPropDef('invalid aggregated collection path.');
				const desc = c.getPropertyDesc(p);
				switch (desc.scalarValueType) {
				case 'object':
					aggColContainerChain.push(desc.nestedProperties);
					break;
				case 'ref':
					aggColContainerChain.push(
						this._recordTypes.getRecordTypeDesc(desc.refTarget));
					break;
				default:
					throw invalidPropDef('invalid aggregated collection path.');
				}
			});
			const aggValueExprCtx = new ValueExpressionContext(
				childNode._aggregatedPropPath, aggColContainerChain);
			childNode._valueExpr = new ValueExpression(
				aggValueExprCtx, valueExprSpecParts[1]);
			const aggUsedPropPaths = new Set(
			...
			valuePropsTrees.set(propPath, buildPropsTreeBranches(
				...
				new ValueExpressionContext(
					topNode._path, topNode._containerChain),
				childNode._aggregatedPropPath, aggUsedPropPaths, {
					noWildcards: true,
					noCalculated: true,
					ignoreScopedOrders: true,
					noScopedFilters: true
		...
		const ignoreFiltersOn = new Set(
			options && options.ignoreFiltersOn ?
				Array.from(options.ignoreFiltersOn).concat(propPath) :
				[ propPath ]
		);
		if (propDef.filter && !(
			options && options.ignoreFiltersOn &&
				options.ignoreFiltersOn.has(propPath))) {
			...
			if (propDesc.isScalar() || !propDesc.isView() || !propDesc.optional)
				throw invalidPropDef(
					'scoped filters are only allowed on non-scalar, optional' +
						' view properties.');
			...
			childNode._filter = filterBuilder.buildFilter(
				...
				(colPropPath, propPaths) => buildPropsTreeBranches(
					...
					colPropPath, propPaths, {
						...
						noScopedFilters: true,
						includeScopeProp: true,
						ignoreFiltersOn: ignoreFiltersOn
					}
				)[0],
				topNode.desc,
	...
	_cloneWithoutChildren() {
		...
		clone._aggregatedPropPath = this._aggregatedPropPath;
	...
	/**
	 * For an aggregate property, the aggregated collection property path.
	 *
	 * @type {string}
	 * @readonly
	 */
	get aggregatedPropPath() { return this._aggregatedPropPath; }
