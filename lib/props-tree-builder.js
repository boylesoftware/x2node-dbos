...
class PropertyTreeNode {
	 ...
	 *
	 * @param {string} clause Clause to associate the new top node with.
	...
	addChild(
		...
		for (let n = this; n; n = n[PARENT_NODE]) {
			topNode = n;
			n._includedPropPaths.add(propPath);
		}
	...
	debranch(branches) {
		...
		if (this._children) {
			...
			aggregateClusters.forEach(aggregateCluster => {
				if (branch._aggregateChild) {
					branch = this._cloneWithoutChildren();
					branch._expanding = !branch._desc.isScalar();
					branches.push(branch);
				}
				branch._aggregateChild = aggregateCluster[0];
			...
			expandingBranches.forEach(childBranch => {
				while (branch._expandingChild || branch._aggregateChild) {
	...
	addChildNode(childNode) {
		...
		if (childNode._includedPropPaths)
			childNode._includedPropPaths.forEach(v => {
				this._includedPropPaths.add(v);
			});
		this._includedPropPaths.add(childNode._path);

		this._children.set(childNode._desc.name, childNode);

		if (childNode._expanding) {
	...
	return assertSingleBranch(topNode.debranch().map(
		branch => valuePropsTreesArray.reduce((res, pair) => {
			...
			if (!res.includesProp(valuePropPath))
				return res;
			return assertSingleBranch(
				res.combine(valuePropsTree).debranch(), true)[0];
		}, branch)
	), scopePropPath);
}
