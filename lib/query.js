	...
	_addSelectedPropToQueryTree(selectedProp, parentQueryTreeNode, markupCtx) {
		...
		case 'map:object:mono':
			...
			if (propDesc.isMap() && propDesc.keyPropertyName) {
				...
				switch (keyPropDesc.scalarValueType) {
				case 'ref':
					anchor.expr = this._dbDriver.nullableConcat(
						keyPropDesc.refTarget + '#',
						queryTreeNode.tableAlias + '.' + keyColumn);
					break;
				case 'number':
					anchor.expr = this._dbDriver.castToString(
						queryTreeNode.tableAlias + '.' + keyColumn);
					break;
				case 'datetime':
					anchor.expr = this._dbDriver.datetimeToString(
						queryTreeNode.tableAlias + '.' + keyColumn);
					break;
				default:
