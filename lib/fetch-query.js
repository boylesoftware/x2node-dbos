...
class FetchQuery {
	...
	constructor(
		...
		const filter = (filterSpec && filterBuilder.buildFilter(
			recordTypes, recordsPropDesc, baseValueExprCtx,
			[ ':and', filterSpec ], usedPropPaths));
		const filterPropsTree = (filter && propsTreeBuilder.buildSupportingTree(
			recordTypes, recordsPropDesc, baseValueExprCtx, 'where',
			usedPropPaths));
		...
		const orderPropsTree = (order && propsTreeBuilder.buildSupportingTree(
			recordTypes, recordsPropDesc, baseValueExprCtx, 'orderBy',
			usedPropPaths));
		...
		// helper functions
		function combineWithFilter(propsTree) {
			if (!filter)
				return propsTree;
			const propsTrees = propsTree.combine(filterPropsTree).debranch();
			if (propsTrees.length > 1)
				throw new common.X2UsageError(
					'The query filter and/or order uses properties' +
						' that lay on different collection axises.');
			return propsTrees[0];
		}
		function combineWithOrder(propsTree) {
			if (!order)
				return propsTree;
			const propsTrees = propsTree.combine(orderPropsTree).debranch();
			if (propsTrees.length > 1)
				throw new common.X2UsageError(
					'The query filter and/or order uses properties' +
						' that lay on different collection axises.');
		...
		if (selectedPropPatterns) {
			...
			const selectPropsBranches = propsTreeBuilder.buildSelectTreeBranches(
				recordTypes, recordsPropDesc, baseValueExprCtx,
