...
class QueryTreeNode {
	 ...
	 */
	addSelectedProp(selectedProp, markupCtx) {
		...
		case 'scalar:object:mono':
			...
			for (let p of selectedProp.children.values())
				queryTreeNode.addSelectedProp(p, childrenMarkupCtx);
			...
			if (propDef.reverseRefProperty) {
				...
				if (reverseRefPropDef.table) {
					...
					this._addLinkTableScalarRefProp(
						propDesc, reverseRefPropDesc, reverseRefPropDef.column,
				...
				this._addLinkTableScalarRefProp(
					propDesc, propDesc, propDef.parentIdColumn, propDef.column,
					fetch, markupPrefix, refTargetDesc, refTargetIdColumn);
				...
				anchor = {
					expr: this.tableAlias + '.' + propDef.column,
					markup: markupPrefix + propDesc.name +
						(fetch ? ':' : '')
				};
		...
		case 'map:ref:mono':
			...
			if (propDef.reverseRefProperty) {
				...
				this._addLinkTableCollectionRefProp(
					propDesc, propDesc, propDef.parentIdColumn, propDef.column,
					fetch, markupCtx, refTargetDesc, refTargetIdColumn,
					childrenMarkupCtx, selectedProp);
			}
	...
	_addLinkTableScalarRefProp(
		propDesc, linkTablePropDesc, linkTableJoinByColumn, linkTableKeyColumn,
		fetch, markupPrefix, refTargetDesc, refTargetIdColumn) {
		...
		const queryTreeNode = this.createChildNode(
			...
			isVirtual(propDesc.definition, false),
			linkTableJoinByColumn, this.keyColumn,
			linkTableKeyColumn);

		// create anchor/value selector
		const anchor = {
			expr: queryTreeNode.tableAlias + '.' + queryTreeNode.keyColumn,
	...
	_addLinkTableCollectionRefProp(
		propDesc, linkTablePropDesc, linkTableJoinByColumn, linkTableKeyColumn,
		fetch, markupCtx, refTargetDesc, refTargetIdColumn, childrenMarkupCtx,
		selectedProp) {
		...
		let queryTreeNode = this.createChildNode(
			linkTablePropDesc.definition.table, true, isVirtual(propDef, true),
			linkTableJoinByColumn, this.keyColumn,
			linkTableKeyColumn);
		...
		if (fetch) {
			...
			const idSelector = {
				expr: queryTreeNode.tableAlias + '.' + refTargetIdColumn,
				markup: childrenMarkupCtx.prefix + refTargetDesc.idPropertyName
			};
			queryTreeNode.keys.push(idSelector);
			queryTreeNode.props.push(idSelector);
 ...
 * suitable for it.
 */
module.exports.build = function(dbDriver, querySpec) {
	...
	for (let p of recordsProp.children.values())
		if (!p.desc.isId()) // already included
			queryTree.addSelectedProp(p, topMarkupCtx);
