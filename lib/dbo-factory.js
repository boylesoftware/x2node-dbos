...
/**
 * Record collection updates tracker that wraps a chain of nested trackers.
 *
 * @private
 * @memberof module:x2node-dbos
 * @inner
 */
class ChainedRecordCollectionsTracker {

	/**
	 * Create new tracker with an empty chain.
	 */
	constructor() {
	...
	/**
	 * Add nested tracker to the chain.
	 *
	 * @param {module:x2node-dbos.RecordCollectionsTracker} tracker The tracker.
	 */
	addTracker(tracker) {
	...
	recordsUpdated(ctx, recordTypeNames) {
		...
		for (let tracker of this._trackers)
			resPromise = resPromise.then(
				() => Promise.resolve(
					tracker.recordsUpdated(ctx, recordTypeNames)),
...
class DBOFactory {
	...
	constructor(dbDriver, recordTypes) {
		...
		this._recordCollectionsTracker = new ChainedRecordCollectionsTracker();
	}
	...
	/**
	 * Add record collection updates tracker to the factory. Every DBO created by
	 * the factory will report record collection updates to all the trackers
	 * registered using this method. The trackers will be called in the order
	 * they were added to the factory.
	 *
	 * @param {module:x2node-dbos.RecordCollectionsTracker} tracker The tracker.
	 * @returns {module:x2node-dbos~DBOFactory} This factory.
	 */
	addRecordCollectionsTracker(tracker) {

		this._recordCollectionsTracker.addTracker(tracker);
	...
	buildInsert(recordTypeName, record) {
		...
		return new InsertDBO(
			this._dbDriver, this._recordTypes, this._recordCollectionsTracker,
