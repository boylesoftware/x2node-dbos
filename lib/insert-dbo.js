...
class ExecutionContext {
	...
	constructor(dbo, connection, rollbackOnError) {
...
class InsertDBO extends AbstractDBO {
	...
	_addInsert(
		...
		if (container.idPropertyName) {
			...
			switch (idPropDesc.generator) {
			case 'auto':
				insertCmd = new InsertWithGeneratedIdCommand(table, idPropDesc);
				break;
			case 'assigned':
		...
		for (let propName of container.allPropertyNames) {
			...
			if (propDesc.scalarValueType === 'object') {
				...
				if ((propVal === undefined) && propDesc.isGenerated()) {
					...
					// add generator command if custom generator
					if ((typeof propDesc.generator) === 'function') {
	...
	_makePropValSql(propDesc, val) {
		...
		case 'ref':
			...
			if (refTarget !== propDesc.refTarget)
				throw invalidVal();
			if (!this._recordTypes.hasRecordType(refTarget))
				throw invalidVal();
	...
	execute(tx) {
		...
		const ctx = new ExecutionContext(
			this, (hasTx ? tx.connection: tx), wrapInTx);
