...
class InsertDBO extends AbstractDBO {
	...
	execute(txOrCon, actor) {
		...
		// queue up entangled record meta-info updates
		resPromise = resPromise.then(
			() => this._updateEntangledRecords(ctx, this._entangledUpdates),
		...
		resPromise = resPromise.then(
			() => ctx.getGeneratedParam(this._recordTypeDesc.idPropertyName),
			err => Promise.reject(err)
		);
