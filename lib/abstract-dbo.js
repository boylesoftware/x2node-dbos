...
/**
 * Call record collection updates tracker.
 *
 * @private
 * @memberof module:x2node-dbos
 * @inner
 * @implements module:x2node-dbos.DBOCommand
 */
class CallRecordCollectionTrackersCommand {

	constructor(tracker, recordTypeNames) {
		...
		return promiseChain.then(
			() => (
				this._recordTypeNames.size > 0 ?
					this._tracker.recordsUpdated(ctx, this._recordTypeNames) :
					undefined
			),
			err => Promise.reject(err)
		);
...
class AbstractDBO {
	 ...
	 * library.
	 * @param {module:x2node-dbos.RecordCollectionsTracker} [recordCollectionsTracker]
	 * The record collection updates tracker.
	 */
	constructor(dbDriver, recordTypes, recordCollectionsTracker) {
		...
		this._recordCollectionsTracker = recordCollectionsTracker;
		this._updatedRecordTypeNames = (
			recordCollectionsTracker ? new Set() : null);
	...
	/**
	 * Build DBO command that notifies registered record collection updates
	 * trackers about the updated record types. The record type names are
	 * registered by the DBO throughout its execution via the
	 ...
	 * @returns {module:x2node-dbos.DBOCommand} The command.
	 */
	_createCallRecordCollectionTrackersCommand() {

		return new CallRecordCollectionTrackersCommand(
			this._recordCollectionsTracker, this._updatedRecordTypeNames);
	}
