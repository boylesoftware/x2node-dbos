...
class AbstractDBO {
	...
	/**
	 * Queue up meta-info updates for all updated entangled records.
	 *
	 * @protected
	 * @param {module:x2node-dbos~DBOExecutionContext} ctx The operation
	 * execution context.
	 * @param {Object.<string,Set.<(string|number)>>} entangledUpdates Ids of
	 * entangled records by record type names.
	 * @returns {Promise} Completion promise.
	 */
	_updateEntangledRecords(ctx, entangledUpdates) {
		...
		for (let recordTypeName in entangledUpdates) {

			// get the entangled record ids
			const ids = entangledUpdates[recordTypeName];
			if (ids.size === 0)
				continue;

			// get record type descriptor
			const recordTypeDesc = this._recordTypes.getRecordTypeDesc(
				recordTypeName);

			// check if has anything to update and build SET clause
			this._getModificationMetaInfoSets(sets, recordTypeDesc);
			if (sets.length === 0)
				continue;
			...
			const stmt = 'UPDATE ' + recordTypeDesc.table + ' SET ' +
				sets.map(s => `${s.columnName} = ${s.value}`).join(', ') +
				' WHERE ' + (
					ids.size === 1 ?
						idColumn + ' = ' + this._dbDriver.sql(
							ids.values().next().value) :
						idColumn + ' IN (' + Array.from(ids).map(
							v => this._dbDriver.sql(v)).join(', ') + ')'
				);
			...
			resPromise = resPromise.then(
				() => new Promise((resolve, reject) => {
					...
					try {
						...
						this._dbDriver.executeUpdate(ctx.connection, sql, {
							onSuccess() {
								resolve();
							},
							onError(err) {
	 ...
	 *
	 * @protected
	 * @param {Array} [sets] Array to use for the SET clause elements. If
	 * provided, it is cleared before adding the SET clause elements. If not
	 * provided, a new array is created.
	 * @param {module:x2node-records~RecordTypeDescriptor} recordTypeDesc Record
	 * type descriptor.
	 * @param {string} [tableAlias] Alias of the record type main table in the
	 * UPDATE statement, or nothing is not used.
	 * @returns {Array} SET clause elements, or empty array is no record
	 * modification meta-info on the record type.
	 */
	_getModificationMetaInfoSets(sets, recordTypeDesc, tableAlias) {
		...
		let propName = recordTypeDesc.getRecordMetaInfoPropName(
			'version');
		if (propName) {
		...
		propName = recordTypeDesc.getRecordMetaInfoPropName(
