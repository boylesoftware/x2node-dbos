...
/*class LoadTempTableCommand {

	constructor(recordTypeTableName) {

		this._recordTypeTableName = recordTypeTableName;
		//...
	}
	...
	queueUp(promiseChain, ctx) {
		...
		const varName = `x2node.q.${this._recordTypeTableName}`;
		const tempTableName = `q_${this._recordTypeTableName}`;

		promiseChain.then(
			() => new Promise((resolve, reject) => {

				// check if temp table already exists for the session
				ctx.dbDriver.getSessionVariable(
					ctx.connection, varName, 'boolean', {
						onSuccess(varVal) {
							if (varVal === undefined) {
								//...
							} else {
...
class AbstractDBO {
	...
	/**
	 * Build a DBO command that simply executes the specified SQL statement and
	 * add it to the provided commands sequence. The command also calls context's
	 ...
	 * sequence, to which to add the command.
	 * @param {string} stmt The SQL statement (may contain param placeholders).
	 * @param {*} [stmtId] DBO-specific statement id, if any.
	 */
	_createExecuteStatementCommand(commands, stmt, stmtId) {

		commands.push(new ExecuteStatementCommand(stmt, stmtId));
	}
	...
	/**
	 * Build DBO commands used to update entangled records modification meta-info
	 * at the end of the DBO and add them to the provided commands sequence. The
	 * updated entangled records information is taken from the DBO execution
	 * context's
	 ...
	 * @protected
	 * @param {Array.<module:x2node-dbos.DBOCommand>} commands The commands
	 * sequence, to which to add the update commands.
	 */
	_createUpdateEntangledRecordsCommands(commands) {

		commands.push(new UpdateEntangledRecordsCommand());
	}
