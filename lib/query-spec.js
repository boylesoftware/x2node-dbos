...
class QuerySpec {
	...
	constructor(recordTypes, topRecordTypeName, querySpec) {
		...
		// get super record type name
		const superTypeName = this._getSuperRecordTypeName(
			recordTypes, topRecordTypeName);
		...
		this._range = this._parseRangeSpecification(querySpec.range);
	}

	/**
	 * Get super record type name. If super record type has not been registered
	 * yes, then create and register it.
	 *
	 * @prviate
	 * @param {module:x2node-records~RecordTypesLibrary} recordTypes Record types
	 * library.
	...
	/**
	 * Parser raw query range specification.
	 *
	 * @private
	 ...
	 * invalid.
	 */
	_parseRangeSpecification(rangeSpec) {
