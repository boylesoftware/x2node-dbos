...
class FetchDBO extends AbstractDBO {
	...
	constructor(
		...
		if (selectedSuperProps && selectedSuperProps.length > 0) {
			...
			superPropsBranches.forEach(superPropsBranch => {
				...
				this._commands.push(new ExecuteSuperPropertiesFetchCommand(
					recordTypes, superTypeName, queryTree.assembleSelect(
						combinedFilter, null, /***/null, this._paramsHandler)));
			});
		}
			...
			if (range) {
				...
				const idsQuery = dbDriver.makeRangedSelect(
					idsQueryTree.assembleSelect(
						filter, order, /***/null, this._paramsHandler),
				...
				dbDriver.makeSelectIntoTempTable(
					idsQuery, anchorTable, preStatements, postStatements);
				preStatements.forEach(stmt => {
					this._createExecuteStatementCommand(this._commands, stmt);
				});
				...
				selectPropsBranches.forEach(selectPropsBranch => {
					...
					this._commands.push(new ExecuteMainRecordsFetchCommand(
						recordTypes, recordTypeName, queryTree.assembleSelect(
							null, order, /***/null, this._paramsHandler)));
				});

			} else { // multi-branch, not ranged

				// add filter properties to the tree
				const idsPropsTree = combineWithFilter(selectPropsBranches[0]);
				...
				const idsQuery = idsQueryTree.assembleSelect(
					filter, null, /***/null, this._paramsHandler);
			...
			if (range) {
				...
				if (selectPropsTree.hasExpandingChild()) {
					...
					dbDriver.makeSelectIntoTempTable(
						idsQuery, anchorTable, preStatements, postStatements);
					preStatements.forEach(stmt => {
						this._createExecuteStatementCommand(
							this._commands, stmt);
					});
		...
		// add post-statements, if any
		postStatements.forEach(stmt => {
			this._createExecuteStatementCommand(this._commands, stmt);
		});
	}
