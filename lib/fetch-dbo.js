...
class FetchDBO {
	...
	/**
	 * <strong>The constructor is not accessible from the client code. Instances are
	 * created using
	 * [DBOFactory]{@link module:x2node-dbos~DBOFactory}.</strong>
	 *
	...
	execute(tx, filterParams) {
		...
		const executePostStatement = stmt => new Promise((resolve, reject) => {
			...
			try {
				...
				common.error('error executing SQL [' + stmt + ']', err);
				reject(err);
			}
		});
